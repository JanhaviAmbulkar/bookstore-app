// FIX: Use absolute path for .env so it works regardless of where you run the script from
...
const seedData = async () => {
  // FIX: Validate MONGO_URI is loaded before trying to connect
  if (!process.env.MONGO_URI) {
    console.error('❌ MONGO_URI not found. Make sure .env exists in the backend folder.');
    process.exit(1);
  }
  ...
  try {
    ...
    // FIX: Close connection so the process exits cleanly instead of hanging
